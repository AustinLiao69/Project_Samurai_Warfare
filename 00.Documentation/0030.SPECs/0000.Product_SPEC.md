...
└── GameSystems (遊戲系統集合)
    ...
    ├── IntrigueSystem (權謀系統)         -- 0004.封建權謀模組
    ├── MilitarySystem (軍事系統)          -- 0006.戰爭軍備模組
    ├── CouncilSystem (評議系統)           -- 0007.評議模組
    ├── IntelligenceSystem (情報系統)      -- 0008.情報模組
    ├── PostWarSystem (戰後處置系統)       -- 0009.戰後處置模組
    └── RetainerEvalSystem (武將評價系統)  -- 0011.武將評價模組
```
...
- name
- rank
- military        -- 軍事（內部數值，對玩家不顯示）
- strategy        -- 智謀（內部數值，對玩家不顯示）
- politics        -- 政治（內部數值，對玩家不顯示）
- charisma        -- 魅力（內部數值，對玩家不顯示）
- loyalty_level   -- 忠誠度（以文字分級顯示，詳見 0004.封建權謀模組）
- current_forces
